  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative">
        {/* Fade-right hint that more content is scrollable */}
        <div className="pointer-events-none absolute right-0 inset-y-0 w-8 z-10 md:hidden"
          style={{ background: 'linear-gradient(to right, transparent, var(--color-onyx))' }} />
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none -mx-4 px-4 md:mx-0 md:px-0">
          ...
          <select
            ...
            className="shrink-0 rounded-xl border border-white/10 bg-white/5 text-sm text-ivory px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold/50 cursor-pointer min-h-[44px]"
          >
          ...
          {hasFilters && (
            <button
              onClick={clearAll}
              className="shrink-0 flex items-center gap-1.5 rounded-xl border border-white/10 px-3 py-2.5 text-sm text-platinum hover:text-ivory hover:border-white/20 transition-colors min-h-[44px]"
            >
              <X size={13} /> Clear
            </button>
          )}
